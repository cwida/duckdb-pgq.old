...
def read_and_replace(mode, thread_file='', sf_file='', timing_file='', timing_output_file='', lane=64, thread_count=1,
                     sf_count=0.1, divisor=2):
    if (mode == Mode.THREAD):
    ...
    for file in thread_files:
        ...
        for thread_count in thread_values:
            ...
            for run in range(nruns):
                ...
                if debug:
                    debug_file = "sf" + str(sf) + "_" + "lane" + str(lane_count) + "_" + "thread" + str(
                        thread_count) + "_variant" + str(variant) + "_run" + str(run) + ".out"
                ...
                output_file = output_base + '_' + "lane" + str(lane_count) + '_' + "vertex" + str(
                    vertex_count) + '_' + "edge" + \
                              str(edge_count) + '_' + "thread" + str(thread_count) + "_variant" + str(
                    variant) + '_run' + str(run) + ".out"
    ...
    for lane in lane_values:
        ...
        for thread_count in thread_values:
            ...
            for runs in range(nruns):
                thread_file = os.path.join(BENCHMARK_DIR, file)
                read_and_replace(Mode.THREAD, thread_file=thread_file, thread_count=thread_count)
                output_file = output_base + '_' + "lane" + str(lane) + '_' + "vertex" + str(
                    vertex_count) + '_' + "edge" + \
                              str(edge_count) + '_' + "thread" + str(thread_count) + "_variant" + str(
                    variant) + '_run' + str(run) + ".out"
                run_benchmark(thread_file, output_dir, output_file)
...
for sf in sf_values:
    for f in sf_files:
        sf_file = os.path.join(f) # BENCHMARK_DIR
